...
public class ModelDB {
		...
		try {
			...
			if(rs != null) {
				while (rs.next()) {
					...
					//ajouter des enrégistrement dans la colelction de course
					listCourse.add(new Course(nom, type, nbrSpectateur, recupCircuit(idCircuit) ));	
				}
				rs.close(); 
			...
			System.out.println("Probl�me de r�cup�ration des donn�es : " + " " + erreur.getMessage());
		}
    ...
    public Circuit recupCircuit(int idCircuit){
		...
		try {
			statement = connexion.prepareStatement("SELECT * FROM Course WHERE idCircuit = ?");
			statement.setInt(1, idCircuit);
			rs = statement.executeQuery();
